...
\nocite{icsid:2014}
Our key independent variable measures the number of ICSID disputes a country has faced and to construct this we turn to the publicly maintained records that ICSID provides.\footnote{\citet[p. 7, 10]{icsid:2015}} Using these records, we create three versions of the disputes variable, a counter of how many disputes a country has faced in the past two years, in the past five years, and a cumulative count. Given our arguments in the preceding section, our expectation is that each of these variables will have a minimal and uncertain effect on FDI flows.\footnote{We do not advance any hypotheses about the effects of winning or losing disputes because a high proportion of registered ICSID cases (39 percent at the ICSID as of late 2013) have not been concluded, and there is no reason to think that those belonging to the concluded set are representative of the broader universe. As suggested above, ``concluded'' is also a potentially misleading label even as applied to ICSID cases inasmuch as a dispute decided by an arbitral panel at one point in time is subject to further revision and annulment proceedings as well as supplementary annulment proceedings. According to the ICSID's caseload statistics, 28 percent of all awards rendered by the ICSID between 1972 and 2013 led to applications for annulment\citep[p. 17]{icsid:2014}. The distinction between ``concluded'' and ``pending'' cases is accordingly rather blurry.} 

Before moving towards adding in our set of control variables, we run a simple analysis to determine the bivariate relationship between our dispute variables and logged, FDI flows. To do this, we take every country that had at least one dispute and run a simple linear model with logged, FDI flows as our dependent variable and a lagged version of the dispute measure as the sole independent variable -- we do this separately for the three dispute measures.\footnote{For each of these models we have 29 observations for every country.} The results are shown in Figure~\ref{fig:corrFDI}. In each of these panels, the black points represent the effect that a dispute is expected to have on FDI flows for a particular dispute variable and country during the period of 1986 to 2014. If disputes had costly effects on FDI flows, we would expect to see the majority of points to fall below the red line in these panels, as this would indicate a negative relationship between disputes and FDI flows. However, we find the exact opposite for the vast majority of countries. We also run a model across the full panel of 112 lower income countries, here we include both countries that had disputes and did not, using fixed effects for each country, and the dispute coefficient results of this analysis are included in blue on the far right of each panel. There again we find little evidence of disputes having an adverse effect on FDI flows in this still simple bivariate analysis. 

% \begin{table}[ht]
% \centering
% \begin{tabular}{lccc}
% ~ & ICSID (past 2 years) & ICSID (past 5 years) & Cumulative ICSID$_{t-1}$ \\ \hline
% $\rho_{FDI, Disputes}$ & 0.039 & 0.050 & 0.039 \\
% \end{tabular}
% \caption{Correlation coefficient between disputes and logged, FDI flows across a sample of 109 non-upper income countries from 1986 to 2014.}
% \label{tab:aggCorr}
% \end{table}
...
Building on the theoretical arguments outlined above about the narrow, uncertain, and limited information conveyed by ISDS processes, we hypothesize that the reputational impact of state involvement in an international investor-state dispute is at best limited. Since the institutional edifice upon which the arbitration system is built provides little reliable information to investors, we expect this hypothesis to hold across, regardless of dispute venue. Nevertheless, to take advantage of the theoretical leverage that variations in institutional rules offer with respect to our argument about information flows, we investigate potential differences in the reputational impact of  ICSID versus  non-ICSID dispute settlement. To the extent that information matters, we expect that the institutional visibility, scope of legal authority, and transparency of the ICSID relative to other arbitral venues enhance the reputational costs of dispute involvement at ICSID. Additionally, to the extent that dispute involvement has reputational effects, we expect these to revolve around cumulative dispute involvement. Hence whereas an individual dispute conveys limited information to the international investment community, we hypothesize that an extended record of alleged treaty violations accumulated over time is likely to yield greater adverse reputational consequences. 
...
For the variables of central theoretical interest, changes in dispute involvement and lagged levels of accumulated dispute involvement, the coefficients are decidedly weak. In accordance with theoretical expectation, in none of the columns are short-term increases in the number of arbitral claims registered against a state in the prior year statistically significant. In contrast to prior research, we also find that investor-state disputes, whether registered at the ICSID or all arbitral forums combined, have no reputational impact over the 1987-2006 period. Nevertheless, the results offer some evidence that accumulated dispute involvement at the comparatively transparent ICSID has begun to affect reputation in recent years. With the analysis extended to cover the post-2006 period, which roughly doubles the number of disputes included in the analysis, the coefficients for cumulative dispute involvement achieves statistical significance at the .05 level. This finding, which holds across both sets of data on disputes, helps to underline the importance of information availability and the relative transparency of dispute settlement processes for reputational sanctioning. 
% \newpage
...
\begin{tabular}{lr@{} lr@{}lr@{}lr@{}lr@{}}
	\hline\hline
	~ & \multicolumn{4}{c}{ICSID} & \multicolumn{4}{c}{All Treaty} \\
